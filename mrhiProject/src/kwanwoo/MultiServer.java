...
public class MultiServer {
  ...
  public void init(String id, ArrayList<String> idList, JComboBox cbox, JTextArea tArea) {
    this.idList = idList;
    try {
      ...
      while (true) {//서버가 실행되는 동안 클라이언트들의 접속을 기다림.
        ...
        //클라이언트 정보(ip,port)출력
        Thread msr = new MultiServerRec(id, socket, cbox, tArea);//쓰레드생성
        msr.start();//쓰레드사용
      }
    } catch (Exception e) {
      e.printStackTrace();
  ...
  class MultiServerRec extends Thread {
    ...
    JComboBox cbox;
    HashMap usertAreaList;
    String id;

    //생성자
    public MultiServerRec(String id, Socket socket, JComboBox cbox, JTextArea tArea) {
      usertAreaList = new HashMap();
      String[] arr = new String[2];
      this.socket = socket;
      this.cbox = cbox;
      usertAreaList.put(tArea, id);
      this.id = id;
    ...
    public void run() {//thread를 사용하기위해서 run메소드 재정의
      ...
      try {
        ...
        name = arr[0].trim();
        String chat = arr[1].trim();
        if (name.equals("root")) {
          JTextArea root = (JTextArea) usertAreaList.get("root");
          root.append(cbox.getSelectedItem() + "님에게 : " + chat + "\n");
          System.out.println(chat);
          JTextArea user = (JTextArea) usertAreaList.get(cbox.getSelectedItem() + "");
          user.append("[관리자] 님께서 : " + chat + "\n");
        }
        else {
          cbox.addItem(name);
          JTextArea user = (JTextArea) usertAreaList.get(name);
          user.append("[관리자] 님에게 : " + chat);
        }
